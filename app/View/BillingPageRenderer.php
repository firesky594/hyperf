...
final class BillingPageRenderer
{
 ...
 /** 执行 `adminConfirmations` 方法对应的业务处理。 */
 ...
 /** 执行 `supplierSettlements` 方法对应的业务处理。 */
 public function supplierSettlements(array$s,array$r):string{return$this->shell('供应商结算','<h1>供应商结算</h1>'.$this->rows($r,'当前没有结算单。'));}
 /** 把数据库查询结果统一转换为数组列表。 */
 private function rows(array$r,string$empty):string{if($r===[])return'<p>'.$empty.'</p>';$html='';foreach($r as$row)$html.='<pre>'.$this->e(json_encode($row,JSON_UNESCAPED_UNICODE|JSON_THROW_ON_ERROR)).'</pre>';return$html;}/** 转义 HTML 特殊字符，防止页面注入。 */
