...
private function id(string$k,bool$q=false):int{$v=$q?$this->request->query($k):$this->request->input($k);if((is_int($v)&&$v>0)||(is_string($v)&&preg_match('/^[1-9][0-9]*$/D',$v)))return(int)$v;throw AuthException::badRequest('标识无效。');}/** 读取并校验字符串请求参数。 */
private function str(string$k):string{$v=$this->request->input($k,'');if(!is_string($v))throw AuthException::badRequest('字段无效。');return$v;}/** 执行 `buyer` 方法对应的业务处理。 */
private function buyer(array$s):int{$w=$this->identities->workspace((int)$s['user_id']);return(int)($w['buyer']['id']??0);}/** 执行 `supplier` 方法对应的业务处理。 */
