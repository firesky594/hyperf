...
final class ApplicationController extends AbstractController
{
 ...
 /** 统一执行带校验和异常处理的写操作。 @param callable(array<string,mixed>):ResponseInterface$f */
...
private function session():array{$s=$this->request->getAttribute('user_session');if(!is_array($s))throw AuthException::invalidCredentials();return$s;}/** 读取当前会话的 CSRF 令牌。 */
private function csrf(array$s):string{$v=$s['csrf_token']??'';return is_string($v)?$v:'';}/** 读取并校验字符串请求参数。 */
private function str(string$k):string{$v=$this->request->input($k,'');if(!is_string($v))throw AuthException::badRequest('字段格式错误。');return$v;}/** 读取并校验正整数标识。 */
private function id(string$k):int{$v=$this->request->input($k);if((is_int($v)&&$v>0)||(is_string($v)&&preg_match('/^[1-9][0-9]*$/D',$v)===1))return(int)$v;throw AuthException::badRequest('标识无效。');}
 /** 执行 `buyerId` 方法对应的业务处理。 */
 private function buyerId(array$s):int{$w=$this->identities->workspace((int)($s['user_id']??0));if($w['buyer']===null)throw AuthException::badRequest('采购身份不存在。');return(int)$w['buyer']['id'];}/** 执行 `supplierId` 方法对应的业务处理。 */
