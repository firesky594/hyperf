...
5. [进行中] 将 `progress.md` 作为唯一设计来源提交，记录提交号。
6. [待执行] 直接在 `progress.md` 中追加测试驱动的详细实施任务，然后从第一个未完成任务分步执行。
...
- 当前阶段：阶段 1 / 提交 `progress.md` 唯一设计来源。
- 最后完成：已将完整设计合并到本文档，删除 `docs/` 旧文档，并通过占位符、矛盾、歧义、范围和 `git diff --check` 自检。
- 下一步：单独提交 `progress.md`，然后将提交号和详细实施任务追加回本文档。
...
- 未解决：详细的测试驱动实施任务尚未追加，业务代码尚未开始修改。
- 下一个唯一步骤：完成阶段 1 步骤 5，提交 `progress.md`。
