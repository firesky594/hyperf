...
- 当前分支：`main`。
- 当前阶段：M0 至 M7 已全部完成。
- 最近业务提交：`f671737 功能：完成分布式网关限额与幂等计量`；M7 待本次提交。
- 最近完整验证：`composer test` 为 179 tests / 1199 assertions；`composer analyse` 为 0 errors。
- 数据库状态：运行容器已成功执行管理员、身份、商品、应用、计量和账单 Schema；26 项系统权限已同步；2026-07 空账期任务成功。
...
1. [x] Web 先行：采购账单、付款凭证、平台确认、供应商结算页。
2. [x] 新增账单、账单项、付款凭证、佣金配置、结算单和永久审计表及 Schema 测试。
3. [x] 按账期和计量快照幂等出账，账单生成后不可改写历史价格/佣金。
4. [x] 固定宽限期、期满暂停、确认付款恢复；任务以租约 + 唯一键防重复。
5. [x] 供应商佣金计算、应结金额、人工确认与永久审计。
6. [x] 完整测试、静态分析、端到端关键流程、中文提交并推送。
...
M0 至 M7 已全部完成。后续只接受新的明确任务，不自行扩展里程碑。
...
- M7 完成：采购账单、真实付款凭证上传、管理员付款确认、供应商结算页面和六张账务表闭环。
- M7 快照：账单项固化调用量、单价、佣金基点、平台佣金和供应商净额；后续商品价格与佣金配置不回写历史。
- M7 幂等与状态：账期任务使用 MySQL 命名租约和账期唯一键；固定到期/宽限时间，逾期同事务暂停订阅并审计，确认付款同事务恢复。
- M7 安全 review：付款凭证仅接受 PDF/JPEG/PNG 文件魔数，最大 5 MB，随机文件名；数据库失败时清理孤儿文件；所有管理操作 POST + CSRF + RBAC + 永久审计。
- M7 验证：定向 4 tests / 32 assertions；最终完整回归 179 tests / 1199 assertions；PHPStan 0 errors；`git diff --check` 无输出。
- M7 运行验收：修复 MySQL 多 TIMESTAMP 默认值兼容后 `uniapi:billing-schema` 成功；权限同步新增 4 项；`uniapi:billing-run 2026-07` 成功；双侧未认证路由分别跳转用户/管理员登录页。
- M0-M7 最终断点：全部里程碑已完成，无未执行编号步骤。
